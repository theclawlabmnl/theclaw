...
export default function AdminBookingEditForm({
   ...
   *
   * Always calculated from the services currently
   * selected in the edit form.
   */
   ...
   * Recalculate it from the CURRENT service
   * total whenever services change.
   */
   ...
   * REMAINING
   *
   * Existing verified payments are deducted
   * from the CURRENT discounted total.
   */
  ...
  /*
   * Keep Balance Payment amount synchronized
   * with the current remaining balance.
   */
  useEffect(() => {
    if (
      paymentType !== "balance" ||
  ...
  useEffect(() => {
    ...
    async function loadAvailability() {
      ...
      try {
        ...
        /*
         * Keep the current booking's original
